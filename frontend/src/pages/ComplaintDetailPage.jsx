...
export default function ComplaintDetailPage() {
  ...
  const handleFindSimilar = async () => {
    ...
    try {
      const res = await getComplaintsSimilarTo(id, 8, 0.35);
      setSimilarComplaints(res.results || []);
    } catch (err) {
      console.error('Failed to find similar complaints:', err);
    } finally {
  ...
  return (
    <div className="space-y-6 pb-12 animate-fade-in">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-bold bg-gradient-to-r from-purple-600 to-brand-600 hover:from-purple-500 hover:to-brand-500 text-white shadow-lg shadow-purple-600/20 transition-all disabled:opacity-50"
          >
            <Sparkles className={`w-3.5 h-3.5 ${loadingSimilar ? 'animate-spin' : ''}`} />
            <span>Find complaints similar to this one</span>
          </button>
      ...
      {/* Find Complaints Similar to This One Panel */}
      {showSimilarSection && (
        <div className="glass-panel p-5 rounded-2xl border border-purple-500/30 space-y-4 shadow-xl bg-purple-950/10 animate-fade-in">
          <div className="flex items-start justify-between flex-wrap gap-3 pb-3 border-b border-purple-500/20">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <Sparkles className="w-4 h-4 text-purple-400" />
                <h3 className="text-sm font-black text-purple-300">
                  Complaints Semantically Similar to This Ticket
                </h3>
              </div>
              <p className="text-xs text-slate-400">
                Retrieved using Sentence Transformers dense embeddings + vector similarity. Discovers related complaints even when phrasing differs completely.
              </p>
            </div>
            <button
              onClick={() => setShowSimilarSection(false)}
          ...
          {loadingSimilar ? (
            <div className="py-6 text-center text-xs text-purple-300 font-mono animate-pulse">
              Computing dense vector similarities across tickets database...
            </div>
            ...
            <div className="space-y-3">
              {similarComplaints.map((item) => (
                <div
                  key={item.id}
                  className="p-4 rounded-xl bg-slate-950/80 border border-purple-500/20 hover:border-purple-500/40 flex items-center justify-between flex-wrap gap-3 transition-all"
                >
                  <div className="space-y-1 max-w-xl">
                    <div className="flex items-center gap-2.5 flex-wrap">
                      <span className="font-mono text-xs font-bold text-slate-300">{item.ticket_number}</span>
                      <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold border ${
                        item.similarity_score >= 0.85
                          ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40 shadow-sm'
                          : 'bg-cyan-500/20 text-cyan-300 border-cyan-500/40'
                      }`}>
                        {Math.round(item.similarity_score * 100)}% Semantic Match
                      </span>
                      <DepartmentBadge department={item.category} />
                      <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-900 text-slate-400 border border-slate-800">
                        {item.status}
                      </span>
                    </div>
                    <p className="text-xs font-bold text-white mt-1">{item.subject}</p>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => handleLinkDuplicate(item.id)}
                      disabled={duplicateActionLoading}
                      className="px-3 py-1.5 rounded-lg text-xs font-bold bg-amber-600/80 hover:bg-amber-600 text-white flex items-center gap-1 transition-colors"
                    >
                    ...
                    <button
                      ...
                      disabled={duplicateActionLoading}
                      className="px-3 py-1.5 rounded-lg text-xs font-bold bg-indigo-600/80 hover:bg-indigo-600 text-white flex items-center gap-1 transition-colors"
                    >
            ...
            <div className="py-6 text-center text-xs text-slate-500">
              No semantically similar tickets exceeded the similarity threshold (0.35).
            </div>
          )}
        </div>
      )}
